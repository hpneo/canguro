...
Model.defineSchema = function(modelConstructor) {
  var schema = JSON.parse(global.window.localStorage.getItem('schema:' + modelConstructor.tableName)) || [];
  modelConstructor.attributes = [];

  schema.forEach(function(column) {
    modelConstructor.attributes.push(column['name']);
...
Model.define = function(modelName, modelOptions) {
  var ModelConstructor = function Model(attributes) {
    ...
    for (var i in attributes) {
      if (this.constructor.attributes.indexOf(i) > -1) {
