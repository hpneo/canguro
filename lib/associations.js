  ...
  Object.defineProperty(modelConstructor.prototype, propertyName, {
    ...
    get: function() {
      ...
      if (this.associations.hasMany[name] === undefined) {
        ...
        var conditions = {};
        conditions[Inflector.singularize(modelConstructor.tableName) + '_id'] = this.id;

        this.associations.hasMany[name] = new Relation({
          owner: this,
          model: Canguro.mappedModels[tableName],
          table: name,
          fields: [Inflector.singularize(modelConstructor.tableName) + '_id', 'id'],
          values: {
            where: conditions
          }
        });
  ...
  Object.defineProperty(modelConstructor.prototype, propertyName, {
    ...
    set: function(record) {
      ...
      if (record.id !== undefined) {
        this[Inflector.singularize(name) + '_id'] = record.id;
      }

      var conditions = {};
      conditions.id = this[Inflector.singularize(name) + '_id'];

      this.associations.belongsTo[name] = new Relation({
        owner: this,
        model: Canguro.mappedModels[tableName],
        table: tableName,
        fields: ['id', Inflector.singularize(name) + '_id'],
        records: [record],
        values: {
          where: conditions,
          oneResult: true
