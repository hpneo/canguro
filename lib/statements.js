...
Statements.createTable = function(tableName, columns, tableOptions) {
  ...
  global.window.localStorage.setItem('schema:' + Canguro.options.name + ':' + tableName, JSON.stringify(columnOptions));
...
Statements.addColumn = function(tableName, columnName, columnType) {
  ...
  // Move this to Migration
  var tableSchemaInStorage = JSON.parse(global.window.localStorage.getItem('schema:' + Canguro.options.name + ':' + tableName));
  ...
  global.window.localStorage.setItem('schema:' + Canguro.options.name + ':' + tableName, JSON.stringify(tableSchemaInStorage));
...
Statements.dropColumn = function(tableName, columnName) {
  ...
  tableSchemaInStorage = tableSchemaInStorage.filter(function(column) {
    return column.name !== columnName;
  });

  global.window.localStorage.setItem('schema:' + Canguro.options.name + ':' + tableName, JSON.stringify(tableSchemaInStorage));

  return table.alter().dropColumn(columnName).toQuery().text;
};
...
Statements.storageToSQL = function(tableName) {
  var tableSchema = JSON.parse(global.window.localStorage.getItem('schema:' + Canguro.options.name + ':' + tableName));
