  ...
  relation.values.where.forEach(function(condition) {
    if (typeof condition === 'string') {
      ...
      if (Array.isArray(condition[1])) {
        query = query.where(table[condition[0]].in(condition[1]));
      }
      else {
...
Relation.prototype.destroyAll = function() {
  var promise = this.load().then(function(records) {
    ...
    return global.window.Promise.all(promises);
  });
